...
function (matcher::BracketMatcherSettings)(ansitokens::Vector{ANSIToken}, tokens::Vector{Token}, cursorpos::Int)
...
function bracket_match(tokens::Vector{Token}, cursoridx::Int)
	...
	println(tokens)
	println(enclosing_token_idx)

	if enclosing_token_idx == -1
		return 0, 0, false
	end
	println("$(tokens[enclosing_token_idx])")
	...
	for idx in enclosing_token_idx:-1:0
		...
		if (i = findfirst(LEFT_DELIMS, kind(token))) != 0
			if counts[i] == 0
				println("Found $(tokens[idx]) at $idx")
				left_match = i
				left_idx = idx
				break
			else
